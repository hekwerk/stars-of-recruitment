...
const handler: AWSLambda.Handler = async (event, context, callback) => {
  ...
  if (!accessToken) {
    ...
    const Linkedin = new LinkedInRestClient(linkedInClientId, linkedInClientSecret, callbackUrl);
    try {
      const result = await Linkedin.getCurrentMemberProfile(['id'], accessToken);
      callback(null, { statusCode: 200, body: JSON.stringify(result) });
    }
    catch (err) {
      callback(null, { statusCode: err?.status || 400, body: err?.message || 'Unknown error'});
    }
  }
}
